...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"features_{args.frames}"):
        print("making folder")
        os.mkdir(f"features_{args.frames}")
    ...
    import torch
    for q_uid in os.listdir(f"./features_{args.frames}/"):
        if torch.from_numpy(np.load(f"./features_{args.frames}/{q_uid}").astype("float32")).shape[0] != frames:
            to_add.append(q_uid[:q_uid.rfind('.')])
            print(q_uid)
    ...
    for q_uid_mp4 in os.listdir(f"{VID_PATH}"):
        ...
        if q_uid not in to_add:
            continue
        row = [f"{VID_PATH}/"+ q_uid_mp4, f"./features_{args.frames}/" + q_uid]
        table.append(row)
