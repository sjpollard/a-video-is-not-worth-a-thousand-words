...
from internvideo.InternVideo1.Downstream.multi_modalities_downstream.CoTrain.transforms import keys_to_transforms
import decord
...
import string
from internvideo.InternVideo1.Downstream.multi_modalities_downstream.CoTrain.transforms.video.videoaug import VideoTransform, video_aug
from internvideo.InternVideo1.Downstream.multi_modalities_downstream.CoTrain.datasets import client
...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def _get_video_path(self, sample):
        if self.names[0] in ['msrvtt_train', 'msrvtt_test', 'msrvtt_val']:
            return os.path.join(self.data_dir, 'videos', sample.name + '.mp4'), sample.name + '.mp4'
        else:
            return os.path.join(self.data_dir, 'videos', str(sample['video_id']) + '.mp4'), str(sample['video_id']) + '.mp4'
