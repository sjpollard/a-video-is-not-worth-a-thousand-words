...
# LongVA imports
from longva.longva.longva.model.builder import load_pretrained_model
from longva.longva.longva.mm_utils import tokenizer_image_token
from longva.longva.longva.constants import IMAGE_TOKEN_INDEX
...
def load_egoschema_frames(frames, q_uid, q_dict, video_folder):
    video = load_video(f'{video_folder}/videos/{q_uid}.mp4', frames)

    return [video], ['video 1'], -1


def load_hd_epic_frames(frames, q_uids, q_dict, video_folder):
...
LONGVA_WEIGHTS_PATH = "lmms-lab/LongVA-7B"
...
def longva_forward(model, tokenizer, system_prompt, videos, input_keys, question, options, indices, lengths, answer_ids, mode, parse_tags, force_logit=None):
...
def longva_shap(args, model, image_processor, tokenizer, q_uid, question, options, q_dict, qa_answers):
    with torch.no_grad():
        
        videos, input_keys, stride = args.config['load_frames'](args.frames, q_uid, q_dict, args.config['video_folder'])
        videos = list(map(lambda x: image_processor.preprocess(x, return_tensors='pt')['pixel_values'].to(model.device, dtype=torch.float16), videos))
        ...
        fc = lambda x: longva_forward(model, tokenizer, args.config['system_prompt'], videos, input_keys, question, options, x, lengths, answer_ids, args.mode, parse_tags)
...
def prepare_longva(args):
    model_name = 'llava_qwen'
    tokenizer, model, image_processor, _ = load_pretrained_model(LONGVA_WEIGHTS_PATH, None, model_name)
...
def calculate_shap_values(args): 
    ...
    results_path = f'shap_results/longva/{args.subset_path.split(".")[0].split("/")[-1]}{add_to_path}' if args.subset_path is not None else f'shap_results/longva{add_to_path}'
    ...
    model, image_processor, tokenizer = prepare_longva(args)
    ...
    for q_uid in tqdm(q_uids):
        ...
        if args.force_redo or not os.path.exists(f'{results_path}/{q_uid}_{args.iterations}_{args.mode}.csv'):
            ...
            shap_out = longva_shap(args, model, image_processor, tokenizer, q_uid, q, options, q_dict, qa_answers)
...
def longva_logit_difference(args, model, image_processor, tokenizer, q_uid, question, options, q_dict, qa_answers):
    with torch.no_grad():
...
def compare_logit_values(args):
    ...
    model, image_processor, tokenizer = prepare_longva(args)
    longva_logit_difference(args, model, image_processor, tokenizer, args.q_uid, q, options, q_dict, qa_answers)
...
def longva_evaluate(args, model, image_processor, tokenizer, q_uid, question, options, q_dict, masking_direction, masking_logit, masking_mode):
    with torch.no_grad():
        ...
        fc = lambda x: longva_forward(model, tokenizer, args.config['system_prompt'], videos, input_keys, question, options, x, lengths, answer_ids, args.mode, parse_tags, args.force_logit)
...
def evaluate_model(args):
    ...
    for q_uid in tqdm(q_uids):
        ...
        pred = longva_evaluate(args, model, image_processor, tokenizer, q_uid, q, options, q_dict, args.masking_direction, args.masking_logit, args.masking_mode)
...
def benchmark_model(args):
    ...
    for q_uid in tqdm(q_uids):
        ...
        pred = longva_evaluate(args, model, image_processor, tokenizer, q_uid, q, options, q_dict, masking_directions[0], None, None)
            ...
            if masking_direction in ['positive', 'negative']:
                for masking_logit in masking_logits:
                    for masking_mode in masking_modes:
                        pred = longva_evaluate(args, model, image_processor, tokenizer, q_uid, q, options, q_dict, masking_direction, masking_logit, masking_mode)
                        results[masking_direction][masking_logit][masking_mode]['preds'].append(pred)
            else:
                for masking_mode in masking_modes:
                    pred = longva_evaluate(args, model, image_processor, tokenizer, q_uid, q, options, q_dict, masking_direction, None, masking_mode)
...
def save_frames(args):
    ...
    frames_path = ('frames' if args.saved_frames != -1 else 'all_frames') + f'/longva/{args.dataset}'
