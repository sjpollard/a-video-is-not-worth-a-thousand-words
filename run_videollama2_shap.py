...
# VideoLLaMA2 imports
from videollama2.videollama2 import model_init, mm_infer_logits
...
def load_egoschema_frames(frames, q_uid, q_dict, processor, video_folder):
    video = processor['video'](f'{video_folder}/videos/{q_uid}.mp4').cuda()
...
def load_hd_epic_frames(frames, q_uid, q_dict, processor, video_folder):
    ...
    video = processor['video'](np.concatenate(videos)).cuda()
    
    return [video], input_keys, stride
...
def load_mvbench_frames(frames, q_uid, q_dict, processor, video_folder):
    ...
    video = processor['video'](video).cuda()
...
VIDEOLLAMA2_WEIGHTS_PATH = "DAMO-NLP-SG/VideoLLaMA2-7B-16F"
...
def videollama2_forward(model, tokenizer, system_prompt, videos, input_keys, question, options, indices, lengths, answer_ids, mode, parse_tags, force_logit=None):
    ...
    if len(input_keys) > 1:
        system_prompt = f'{len(input_keys)} input videos have been concatenated together. ' + system_prompt
...
def videollama2_shap(args, model, processor, tokenizer, q_uid, question, options, q_dict, qa_answers):
    with torch.no_grad():
        
        videos, input_keys, stride = args.config['load_frames'](args.frames, q_uid, q_dict, processor, args.config['video_folder'])
        ...
        fc = lambda x: videollama2_forward(model, tokenizer, args.config['system_prompt'], videos, input_keys, question, options, x, lengths, answer_ids, args.mode, parse_tags)
...
def prepare_videollama2(args):
    model, processor, tokenizer = model_init(model_path=VIDEOLLAMA2_WEIGHTS_PATH, dataset=args.dataset)
...
def calculate_shap_values(args): 
    ...
    results_path = f'shap_results/videollama2/{args.subset_path.split(".")[0].split("/")[-1]}{add_to_path}' if args.subset_path is not None else f'shap_results/videollama2{add_to_path}'
    ...
    model, processor, tokenizer = prepare_videollama2(args)
    ...
    for q_uid in tqdm(q_uids):
        ...
        if args.force_redo or not os.path.exists(f'{results_path}/{q_uid}_{args.iterations}_{args.mode}.csv'):
            ...
            shap_out = videollama2_shap(args, model, processor, tokenizer, q_uid, q, options, q_dict, qa_answers)
...
def videollama2_logit_difference(args, model, processor, tokenizer, q_uid, question, options, q_dict, qa_answers):
    with torch.no_grad():
...
def compare_logit_values(args):
    ...
    model, processor, tokenizer = prepare_videollama2(args)
    videollama2_logit_difference(args, model, processor, tokenizer, args.q_uid, q, options, q_dict, qa_answers)
...
def videollama2_evaluate(args, model, processor, tokenizer, q_uid, question, options, q_dict, masking_direction, masking_logit, masking_mode):
    with torch.no_grad():
        ...
        fc = lambda x: videollama2_forward(model, tokenizer, args.config['system_prompt'], videos, input_keys, question, options, x, lengths, answer_ids, args.mode, parse_tags, args.force_logit)
...
def evaluate_model(args):
    ...
    for q_uid in tqdm(q_uids):
        ...
        pred = videollama2_evaluate(args, model, processor, tokenizer, q_uid, q, options, q_dict, args.masking_direction, args.masking_logit, args.masking_mode)
...
def benchmark_model(args):
    ...
    for q_uid in tqdm(q_uids):
        ...
        pred = videollama2_evaluate(args, model, processor, tokenizer, q_uid, q, options, q_dict, masking_directions[0], None, None)
            ...
            if masking_direction in ['positive', 'negative']:
                for masking_logit in masking_logits:
                    for masking_mode in masking_modes:
                        pred = videollama2_evaluate(args, model, processor, tokenizer, q_uid, q, options, q_dict, masking_direction, masking_logit, masking_mode)
                        results[masking_direction][masking_logit][masking_mode]['preds'].append(pred)
            else:
                for masking_mode in masking_modes:
                    pred = videollama2_evaluate(args, model, processor, tokenizer, q_uid, q, options, q_dict, masking_direction, None, masking_mode)
...
def save_frames(args):
    ...
    frames_path = ('frames' if args.saved_frames != -1 else 'all_frames') + f'/videollama2/{args.dataset}'
    ...
    for q_uid in q_uids:        
        ...
        videos, input_keys, stride = args.config['load_frames'](args.frames, q_uid, q_dict, processor, args.config['video_folder'])
        videos = list(map(lambda x: x.cpu() , videos))
        ...
        frames = list(map(lambda x: Image.fromarray((x * 255).astype(np.uint8)), frames))
