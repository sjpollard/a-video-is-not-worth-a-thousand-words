...
from .mm_utils import process_image, process_video, process_video_hd_epic, tokenizer_multimodal_token, get_model_name_from_path, KeywordsStoppingCriteria
...
def model_init(model_path=None, **kwargs):
    ...
    # Modified by Sam Pollard
    if dataset in ['egoschema', 'mvbench', 'lvbench']:
        processor = {
        ...
        processor = {
            'image': partial(process_image, processor=processor, aspect_ratio=None),
            'video': partial(process_video_hd_epic, processor=processor, aspect_ratio=None, num_frames=num_frames),
        }

    return model, processor, tokenizer

# Modified by Sam Pollard
def mm_infer_logits(image_or_video, instruct, model, tokenizer, system_prompt, modal='video', **kwargs):
